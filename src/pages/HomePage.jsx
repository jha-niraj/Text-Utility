...
const Card = ({ description, path, to, theme }) => {
    ...
    return (
        <section className="shadow-2xl p-3 w-full flex flex-col gap-4 rounded-lg items-center border-2 border-gray-200 justify-between">
            ...
            <div>
                <button onClick={() => navigate(`/${path}`)} className={`${theme === "black" ? "bg-white text-black" : "bg-black text-white"} hover:bg-slate-700 hover:text-white font-bold py-2 px-4 rounded transition-all duration-300`}>{to}</button>
            </div>
        </section>
    )
}